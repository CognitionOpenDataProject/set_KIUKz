...
# keep only 1 row per subject, 1 for each task
...
succeeded = function( f.count, h.count, nf, nh ){
  return( prop.test( x=c(f.count, h.count), n=c(nf, nh) )$p.value < 0.05 )
}
dt2[, success := succeeded( f.count=n.fa, h.count=n.hits, nf=n.distr, nh=n.targets) ]

# look at success by condiiton
aggregate( success ~ Task + InvCond, dt2, mean)
# no one appears to have failed anything
# we were supposed to get 3 exclusions in the upright FFR category
# ~~~~~~~PROBLEM
...
```{r}
# means by condition
t1 = aggregate( d ~ Task + InvCond, dt2, mean)

t2 = aggregate( d ~ Task + InvCond, dt2, sd)
```
...
```{r}
aov( d ~ (Task * InvCond) + Error(subID / (Task * InvCond)), data=dt2 )
```


```{r}
dprime.aov = aov( dprime ~ (Task * InvCond) + Error(subID / (Task * InvCond) ), data=data.frame(dt2) )
